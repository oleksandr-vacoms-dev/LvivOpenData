...
import com.vakoms.oleksandr.havruliyk.lvivopendata.ui.adapter.viewholder.MarketItemViewHolder
...
class MarketItemAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            R.layout.item_list -> MarketItemViewHolder.create(parent, onItemClickListener)
    ...
    companion object {
        private val ITEM_COMPARATOR = object : DiffUtil.ItemCallback<MarketRecord>() {
            override fun areItemsTheSame(oldItem: MarketRecord, newItem: MarketRecord) =
                (oldItem.id == newItem.id)

            override fun areContentsTheSame(oldItem: MarketRecord, newItem: MarketRecord) =
                (oldItem == newItem)
        }
    }
}